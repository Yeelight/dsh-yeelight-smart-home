...
function detectInstallOptions(env) {
  const platform = process.platform;
  const options = [];
  const has = (name) => findOnPath(name, env) !== void 0;
  if (platform === "darwin") {
    options.push({
      channel: "brew",
      label: "Homebrew",
...
function runStream(command, args, options) {
  return new Promise((resolve2) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        resolve2({ ok: false, code: null, stdout, stderr, timedOut: true });
      }
    ...
    child.on("error", (error) => {
      if (!settled) {
        settled = true;
        clearTimeout(timer);
        resolve2({ ok: false, code: null, stdout, stderr: `${stderr}${error.message}`, timedOut: false });
      }
    });
    child.on("close", (code) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      resolve2({ ok: code === 0, code, stdout, stderr, timedOut: false });
    });
    if (options.stdin !== void 0) child.stdin.write(options.stdin);
    child.stdin.end();
  });
}
function localBinPath(env) {
  const fromEnv = (env.YEELIGHT_HOME_BIN ?? "").trim();
  if (fromEnv !== "") return fromEnv;
  return join8(homedir3(), ".local", "bin", "yeelight-home");
}
async function installRuntime(env, options) {
  const report = (phase, message, output) => options.onProgress?.({ phase, message, output });
  ...
  if (chosen.channel === "download") {
    ...
    result = {
      ...
      ...captured.timedOut ? { error: `install timed out after ${options.timeoutMs ?? 12e4}ms` } : {},
  ...
  const version = await captureCommand(bin, ["version", "--json"], { timeoutMs: 1e4, env });
  const parsed = JSON.parse(version.stdout || "{}");
  const versionText = typeof parsed.version === "string" ? parsed.version : void 0;
  report("done", `\u5B89\u88C5\u6210\u529F\uFF1Ayeelight-home ${versionText ?? ""} @ ${bin}`.trim(), result.output);
  return { ...result, bin, version: versionText };
}
async function installFromGithub(env, options) {
  ...
  try {
    ...
    if (process.platform !== "win32") chmodSync(target, 493);
...
async function dispatch(req, res, service) {
  ...
  switch (path) {
    ...
    case "/yeelight/debug/describe": {
      ...
      const describe = service.settingsDescribe;
      send(res, 200, {
        ok: true,
        value: {
    ...
    case "/yeelight/install": {
      ...
      const body = await readBody(req);
      const progress = [];
      const channel = typeof body?.channel === "string" ? body.channel : void 0;
      if (body?.dry_run === true) {
        const preview = detectInstallOptions(service.env);
        const chosen = channel !== void 0 ? preview.find((o) => o.channel === channel) : preview.find((o) => o.available);
        send(res, 200, { ok: true, value: { dryRun: true, chosen: chosen ?? null } });
        return;
      }
      const result = await installRuntime(service.env, {
        channel,
        timeoutMs: typeof body?.timeout_ms === "number" ? body.timeout_ms : void 0,
...
function apply(ctx, options = {}) {
  ...
  const settingsDescribeRef = { current: void 0 };
  ...
  ctx.inject(["settings"], (scope) => {
    try {
      const settingsScope = scope.settings;
