...
> Port note (dsh-yeelight-smart-home): this skill runs inside DeepSeek Harness as the
> `dsh-yeelight-smart-home` plugin. The original `scripts/invoke` wrapper below is
> equivalent to the plugin's `yeelight_home` tool (same `yeelight-home invoke --stdin`
> pipeline, same request and response contract), the original file-based
> `references/` loading is equivalent to the `yeelight_reference` tool (same
> documents, keyed by file name), and `node scripts/product-select.mjs` is
> equivalent to the `yeelight_product_select` tool. Wherever this document says
> "call `scripts/invoke`", call the `yeelight_home` tool once instead.

Use only the local `yeelight-home invoke --stdin` runtime through the `yeelight_home` tool (the plugin's wrapper of `scripts/invoke`).
...
2. Load only the relevant document with the `yeelight_reference` tool when the request needs routing detail or domain knowledge. Doc keys are the file names below (without `.md`). If unsure, load `README` first as the shortest-path router:
   - Query, state, capability, temporary device control: `device-control`
   - Product consultation, manual, FAQ, SKU resources, or product pedia: `product-knowledge`
   - Homes, rooms, areas: `home-room-area`
   - Groups: `groups`
   - Scenes, saved action bundles, or scene recipe conversion: `scenes`
   - Automations, schedules, trigger-action rules, or automation design strategy: `automations`
   - Nested action, condition, item, operation, button-event, or machine-readable Runtime schemas: `payload-shapes`; `action-payloads` is only a routing index.
   - Lighting design routing and full-home workflow: `lighting-design`.
   - Standard lighting design import model, future device slots, imported groups, areas, scenes, and automations: `lighting-design-import`.
   - Product candidate selection for not-yet-installed lighting slots: call the `yeelight_product_select` tool with `query` (user product wording), `room`, `goal`, and optional `category`/`limit`, then apply `lighting-product-selection` and `product-knowledge`.
   - Scene recipe conversion: `scene-recipes` plus `lighting-experience` when ambience judgment matters.
   - Automation recipe conversion: `automation-recipes` plus `automation-events` when triggers or condition vocabulary matter.
   - Full multi-room lighting import examples: `yeelight_reference` doc `lighting-design-full-home`. This example is for creating a new home design and intentionally omits `parameters.houseId`.
   - Device, gateway, scene, or automation diagnostics: `diagnostics`
   - Memory or personalization: `memory-and-personalization`
   - Recommendations and feedback: `recommendations`
   - Operation lessons, known pitfalls, fastest paths, repeated Runtime usage failures, parameter-shape learnings, or capability workarounds: `operation-lessons`
   - Delete, unbind, transfer, permission, bulk, mixed configuration, or risky changes: `safety-and-confirmation`
   - Runtime statuses, auth, partial results, retry, cache, or error handling: `runtime-status-and-errors`
   - Blocked capabilities, manual guidance, risk lanes, or non-enabled action classes: `capability-boundaries`
   - Thing model, category, component, property, or capability language: `thing-model`
   - Device families, aliases, product words, typo-prone wording, or fuzzy device mentions: `device-lexicon`
   - Automation event wording, trigger-condition vocabulary, templates, patterns, or anti-patterns: `automation-events`
   - Lighting ambience, scene recipes, compound flows, mood interpretation, or design rules: `lighting-experience`
   - Response presentation, tables, cards, dashboards, notifications, memory-result wording, recommendation-result wording, or product help surfaces: `response-presentation`
...
4. Call the `yeelight_home` tool once: pass `utterance` plus the classified `intent` and `parameters` (or the complete SkillRequest JSON as `json`).
...
9. When any Runtime/Skill usage attempt fails, is blocked, returns unsupported/not_supported, needs a workaround, exposes a parameter-shape trap, wastes turns on resource resolution, or reveals a faster path, evaluate whether it produced reusable operational knowledge using `operation-lessons`. If a later attempt in the same conversation succeeds because you changed intent, target resolution, payload shape, dependency handling, safety lane, or fallback path, save a concise structured lesson with `operation.lesson.record` before finalizing, unless the current Runtime response already provides the clear supported path. Before attempting a complex, parameter-heavy, full-home lighting design, or previously failed capability, query `operation.lesson.list` for the target intent or symptom and apply the returned lesson if it is still relevant. If the user reports a failed AI/Runtime attempt, convert only the confirmed reusable runtime boundary or stable workaround into an operation lesson; do not leave it only in the chat transcript.
...
13. After Runtime returns, choose the user-facing response shape from `response-presentation`. Use tables for lists, cards for single entities or completed operations, dashboards for home summaries and new-home imports, and clear comparison tables for changes.

## Response Style

Use brief natural Chinese for ordinary users, then pick a scan-friendly response shape from `response-presentation`.
